...
namespace lightweight_ur_interface
{
    ...
    public:
        ...
        URPositionController(ros::NodeHandle& nh,
                             ...
                             const bool autoscale_velocities) : nh_(nh)
        {
            ...
            feedback_sub_ = nh_.subscribe(state_feedback_topic, 1, &URPositionController::StateFeedbackCallback, this);
            position_command_sub_ = nh_.subscribe(position_command_topic, 1, &URPositionController::PositionCommandCallback, this);
            abort_server_ = nh_.advertiseService(abort_service, &URPositionController::AbortCB, this);
        }
            ...
            while (ros::ok())
            {
                ...
                if (iteration_count == supersample_rate)
                {
                    if (target_config_valid_ && current_state_valid_)
                    {
                        ...
                        const std::vector<double> config_correction = (autoscale_velocities_) ? AutoscaleCorrection(current_config_, target_config_, raw_config_correction) : raw_config_correction;
                        const std::vector<double> velocity_limited_config_correction = LimitCorrectionVelocities(config_correction);
                        if (limit_acceleration_)
                        {
                            ...
                            PublishState(target_config_, acceleration_limited_config_correction, current_config_, current_velocities_);
                        }
                        else
                        {
                            CommandVelocities(velocity_limited_config_correction);
                            previous_velocity_command = velocity_limited_config_correction;
                            PublishState(target_config_, velocity_limited_config_correction, current_config_, current_velocities_);
                        }
                    }
        ...
        std::vector<double> ComputeRawNextStep(const std::vector<double>& current_config, const std::vector<double>& target_config, const double time_interval)
        {
            ...
            const std::vector<double> config_error_integrals_update = EigenHelpers::Multiply(EigenHelpers::Add(EigenHelpers::Multiply(last_config_errors_, 0.5), EigenHelpers::Multiply(config_errors, 0.5)), time_interval);
            config_error_integrals_ = ComputeNewConfigErrorIntegral(EigenHelpers::Add(config_error_integrals_, config_error_integrals_update));
        ...
        std::vector<double> AutoscaleCorrection(const std::vector<double>& current_position, const std::vector<double>& target_position, const std::vector<double>& raw_correction) const
        {
            assert(current_position.size() == raw_correction.size());
            assert(target_position.size() == raw_correction.size());
            const std::vector<double> position_error = EigenHelpers::Sub(target_position, current_position);
            const double max_position_error = *std::max_element(position_error.begin(), position_error.end());
            ...
            for (size_t idx = 0; idx < autoscaled_correction.size(); idx++)
            {
                ...
                const double axis_error_ratio = std::abs(axis_error / max_position_error);
                const double autoscaled_axis_correction = raw_axis_correction * axis_error_ratio;
        ...
        void PositionCommandCallback(lightweight_ur_interface::PositionCommand config_target)
        {
            if (config_target.name.size() == config_target.position.size())
            {
                ...
                for (size_t idx = 0; idx < joint_names_.size(); idx++)
                {
                    ...
                    {
                        ROS_WARN_NAMED(ros::this_node::getName(), "Invalid PositionCommand: joint %s missing", joint_name.c_str());
                        command_valid = false;
                    }
                }
            ...
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid PositionCommand: %zu names, %zu positions", config_target.name.size(), config_target.position.size());
                target_config_valid_ = false;
            }
        }
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO_NAMED(ros::this_node::getName(), "Starting ur_position_controller...");
    ros::NodeHandle nh;
    ros::NodeHandle nhp("~");
    const std::string DEFAULT_POSITION_COMMAND_TOPIC = "/ur10/joint_command_position";
    const std::string DEFAULT_STATUS_TOPIC = "/ur10_position_controller/status";
    ...
    const std::string position_command_topic = nhp.param(std::string("position_command_topic"), DEFAULT_POSITION_COMMAND_TOPIC);
    ...
    const bool enable_velocity_autoscaling = nhp.param(std::string("enable_velocity_autoscaling"), DEFAULT_ENABLE_VELOCITY_AUTOSCALING);
    // Joint limits
    ...
    lightweight_ur_interface::URPositionController controller(nh, position_command_topic, state_feedback_topic, velocity_command_topic, status_topic, abort_service, joint_limits, joint_controller_params, limit_acceleration, enable_velocity_autoscaling);
