...
#ifndef SCHUNK_WSG_DRIVER_COMMON_HPP
#define SCHUNK_WSG_DRIVER_COMMON_HPP
...
namespace schunk_wsg_driver
{
    ...
    {
        T temp_item;
        if ((current > buffer.size()) || ((current + sizeof(temp_item)) > buffer.size()))
        {
            throw std::runtime_error("Insufficient data to deserialize item");
        }
    ...
    inline uint16_t ComputeCRC(const std::vector<uint8_t>& buffer, const size_t start_idx, const size_t end_idx)
    {
        if ((start_idx >= buffer.size()) || (end_idx > buffer.size()))
        {
            throw std::runtime_error("Start or end index for CRC computation is outside buffer bounds");
        }
        uint16_t crc = 0xFFFF;
    ...
    public:
        ...
        inline uint8_t Command() const { return command_; }

        inline uint16_t Status() const { return status_; }

        inline const std::vector<uint8_t>& ParamBuffer() const { return param_buffer_; }

        static std::pair<WSGRawStatusMessage, uint64_t> Deserialize(const std::vector<uint8_t>& buffer, const uint64_t current);
    ...
    public:
        ...
        WSGRawCommandMessage(const uint8_t command, const std::vector<uint8_t>& param_buffer) : command_(command), param_buffer_(param_buffer) {}

        WSGRawCommandMessage(const uint8_t command) : command_(command), param_buffer_({}) {}
        ...
        void AppendParameterToBuffer(const T& parameter)
        {
            ...
            SerializeFixedSizePOD(parameter, temp_buffer);
            param_buffer_.insert(param_buffer_.end(), temp_buffer.begin(), temp_buffer.end());
        }

        static uint64_t Serialize(const WSGRawCommandMessage& message, std::vector<uint8_t>& buffer);
    ...
    public:
        ...
        inline void UpdateActualVelocity(const double actual_velocity) { actual_velocity_ = actual_velocity; }
    ...
    public:
        ...
        PhysicalLimits(const double stroke_mm, const double min_speed_mm_per_s, const double max_speed_mm_per_s, const double min_acc_mm_per_ss, const double max_acc_mm_per_ss, const double min_force, const double nominal_force, const double overdrive_force) : stroke_mm_(stroke_mm), min_speed_mm_per_s_(min_speed_mm_per_s), max_speed_mm_per_s_(max_speed_mm_per_s), min_acc_mm_per_ss_(min_acc_mm_per_ss), max_acc_mm_per_ss_(max_acc_mm_per_ss), min_force_(min_force), nominal_force_(nominal_force), overdrive_force_(overdrive_force) {}

        PhysicalLimits() : stroke_mm_(0.0), min_speed_mm_per_s_(0.0), max_speed_mm_per_s_(0.0), min_acc_mm_per_ss_(0.0), max_acc_mm_per_ss_(0.0), min_force_(0.0), nominal_force_(0.0), overdrive_force_(0.0) {}

        inline double StrokeMM() const { return stroke_mm_; }

        inline std::pair<double, double> MinMaxSpeedMMPerS() const { return std::make_pair(min_speed_mm_per_s_, max_speed_mm_per_s_); }

        inline std::pair<double, double> MinMaxAccelMMPerSS() const { return std::make_pair(min_acc_mm_per_ss_, max_acc_mm_per_ss_); }
    ...
    public:
        ...
        WSGInterface(const std::function<void(const std::string&)>& logging_fn) : logging_fn_(logging_fn) {}
    ...
    protected:
        ...
        bool EnableRecurringStatus(const GripperCommand command, const uint16_t update_period_ms, const double timeout);

        bool DisableRecurringStatus(const GripperCommand command, const double timeout);

        enum PrePositionStopMode : uint8_t {kPrePositionClampOnBlock=0, kPrePositionStopOnBlock=1};

        enum PrePositionMoveMode : uint8_t {kPrePositionAbsolute=0, kPrePositionRelative=2};

        WSGRawCommandMessage MakePrePositionCommand(const PrePositionStopMode stop_mode, const PrePositionMoveMode move_mode, double width_mm, double speed_mm_per_s) const;

        bool PrePosition(const PrePositionStopMode stop_mode, const PrePositionMoveMode move_mode, double width_mm, double speed_mm_per_s);

        bool PrePositionNonBlocking(const PrePositionStopMode stop_mode, const PrePositionMoveMode move_mode, double width_mm, double speed_mm_per_s);
...
#endif // SCHUNK_WSG_DRIVER_COMMON_HPP
