...
{
    AtiNetCanOemInterface::AtiNetCanOemInterface(const std::function<void(const std::string&)>& logging_fn, const std::string& socketcan_interface, const uint8_t sensor_base_can_id) : logging_fn_(logging_fn), has_active_calibration_(false)
    {
        ...
        // Figure out how much we can write (strncpy copies up to N characters, not coutning termination)
        ...
        const int bind_result = bind(can_socket_fd_, (struct sockaddr *)&can_interface, sizeof(can_interface));
        if (bind_result != 0)
        {
    ...
    Eigen::Matrix<double, 6, 1> AtiNetCanOemInterface::GetCurrentForceTorque()
    {
        ...
        {
            const std::pair<uint16_t, Eigen::Matrix<double, 6, 1>> strain_gauge_data = ReadRawStrainGaugeData();
            const uint16_t status_code = strain_gauge_data.first;
            ParseStatusCode(status_code);
            const Eigen::Matrix<double, 6, 1> strain_gauge_values = strain_gauge_data.second;
            const Eigen::Matrix<double, 6, 1> wrench = active_calibration_matrix_ * strain_gauge_values;
            return wrench;
        }
        else
        {
    ...
    std::pair<uint16_t, Eigen::Matrix<double, 6, 1>> AtiNetCanOemInterface::ReadRawStrainGaugeData()
    {
        const DataElement read_strain_gauges(READ_SG_A);
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_strain_gauges, 0x02, 0.1);
        ...
        const uint16_t status_code = DeserializeFixedSizePOD<int16_t>(response_msg_1.Payload(), 0).first;
        raw_values(0, 0) = (double)DeserializeFixedSizePOD<int16_t>(response_msg_1.Payload(), 2).first;
        raw_values(1, 0) = (double)DeserializeFixedSizePOD<int16_t>(response_msg_1.Payload(), 4).first;
        raw_values(2, 0) = (double)DeserializeFixedSizePOD<int16_t>(response_msg_1.Payload(), 6).first;
        ...
        raw_values(3, 0) = (double)DeserializeFixedSizePOD<int16_t>(response_msg_2.Payload(), 0).first;
        raw_values(4, 0) = (double)DeserializeFixedSizePOD<int16_t>(response_msg_2.Payload(), 2).first;
        raw_values(5, 0) = (double)DeserializeFixedSizePOD<int16_t>(response_msg_2.Payload(), 4).first;
        return std::make_pair(status_code, raw_values);
    }

    bool AtiNetCanOemInterface::LoadNewActiveCalibration(const uint8_t calibration_index)
    {
        const bool can_set_calibration = SetActiveCalibration(calibration_index);
        if (can_set_calibration)
        {
            const Eigen::Matrix<double, 6, 6> raw_calibration_matrix = ReadActiveCalibrationMatrix();
            ...
            active_calibration_matrix_.block<1, 6>(0, 0) = raw_calibration_matrix.block<1, 6>(0, 0) / counts_per_force;
            active_calibration_matrix_.block<1, 6>(1, 0) = raw_calibration_matrix.block<1, 6>(1, 0) / counts_per_force;
            active_calibration_matrix_.block<1, 6>(2, 0) = raw_calibration_matrix.block<1, 6>(2, 0) / counts_per_force;
            active_calibration_matrix_.block<1, 6>(3, 0) = raw_calibration_matrix.block<1, 6>(3, 0) / counts_per_torque;
            active_calibration_matrix_.block<1, 6>(4, 0) = raw_calibration_matrix.block<1, 6>(4, 0) / counts_per_torque;
            active_calibration_matrix_.block<1, 6>(5, 0) = raw_calibration_matrix.block<1, 6>(5, 0) / counts_per_torque;
    ...
    {
        Eigen::Matrix<double, 6, 6> calibration_matrix = Eigen::Matrix<double, 6, 6>::Zero();
    ...
    Eigen::Matrix<double, 1, 6> AtiNetCanOemInterface::ReadActiveCalibrationMatrixRow(const uint8_t row)
    {
        if (row > 5)
        {
            throw std::invalid_argument("Row index must be <= 5");
        }
        const DataElement read_calibration_row(READ_MATRIX_ROW_A, std::vector<uint8_t>{row});
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_calibration_row, 0x03, 0.1);
        Eigen::Matrix<double, 1, 6> matrix_row = Eigen::Matrix<double, 1, 6>::Zero();
        if (response.size() != 3)
        {
            throw std::runtime_error("Failed to read calibration matrix row in timeout");
        }
        const DataElement& response_msg_1 = response[0];
        ...
        if (response_msg_1.Payload().size() != 8)
        {
            throw std::runtime_error("Invalid payload data for read calibration matrix row");
        }
        matrix_row(0, 0) = DeserializeFixedSizePOD<float>(response_msg_1.Payload(), 0).first;
        matrix_row(0, 1) = DeserializeFixedSizePOD<float>(response_msg_1.Payload(), 4).first;
        const DataElement& response_msg_2 = response[1];
        ...
        matrix_row(0, 2) = DeserializeFixedSizePOD<float>(response_msg_2.Payload(), 0).first;
        matrix_row(0, 3) = DeserializeFixedSizePOD<float>(response_msg_2.Payload(), 4).first;
        ...
        matrix_row(0, 4) = DeserializeFixedSizePOD<float>(response_msg_3.Payload(), 0).first;
        matrix_row(0, 5) = DeserializeFixedSizePOD<float>(response_msg_3.Payload(), 4).first;
        return matrix_row;
    }
    ...
    std::string AtiNetCanOemInterface::ReadSerialNumber()
    {
        ...
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_serial_number, 0x01, 0.1);
        if (response.size() != 1)
        {
        ...
        const std::string serial_number(reinterpret_cast<const char*>(response_msg.Payload().data()), 8);
        return serial_number;
    }

    bool AtiNetCanOemInterface::SetActiveCalibration(const uint8_t desired_calibration)
    {
        if (desired_calibration > 15)
        {
            throw std::invalid_argument("Desired calibration index must be <= 15");
        }
        const DataElement set_active_calibration(SET_ACTIVE_CALIBRATION, std::vector<uint8_t>{desired_calibration});
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(set_active_calibration, 0x01, 0.1);
        if (response.size() != 1)
        {
        ...
        if (active_calibration == desired_calibration)
        {
            return true;
        }
        else
    ...
    std::pair<uint32_t, uint32_t> AtiNetCanOemInterface::ReadCountsPerUnit()
    {
        ...
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_counts_per_unit, 0x01, 0.1);
        if (response.size() != 1)
        {
        ...
        const uint32_t force_counts = DeserializeFixedSizePOD<uint32_t>(response_msg.Payload(), 0).first;
        const uint32_t torque_counts = DeserializeFixedSizePOD<uint32_t>(response_msg.Payload(), 4).first;
    ...
    std::pair<uint8_t, uint8_t> AtiNetCanOemInterface::ReadUnitCodes()
    {
        ...
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_unit_codes, 0x01, 0.1);
        if (response.size() != 1)
        {
    ...
    std::vector<uint16_t> AtiNetCanOemInterface::ReadDiagnosticADCVoltages()
    {
        ...
        {
            const DataElement read_adc(READ_ADC_VOLTAGES, std::vector<uint8_t>{adc_index});
            const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_adc, 0x01, 0.1);
            if (response.size() != 1)
            {
            ...
            const uint16_t adc_voltage = DeserializeFixedSizePOD<uint16_t>(response_msg.Payload(), 0).first;
    ...
    bool AtiNetCanOemInterface::SetSensorBaseCanID(const uint8_t new_base_can_id)
    {
        ...
        const DataElement set_base_can_id(SET_BASE_ID_BITS, std::vector<uint8_t>{new_base_can_id});
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(set_base_can_id, 0x01, 0.1);
        if (response.size() != 1)
        {
    ...
    {
        const DataElement set_baud_rate(SET_BAUD_RATE, std::vector<uint8_t>{rate_divisor});
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(set_baud_rate, 0x01, 0.1);
        if (response.size() != 1)
        {
    ...
    std::pair<std::pair<uint8_t, uint8_t>, uint16_t> AtiNetCanOemInterface::ReadFirmwareVersion()
    {
        const DataElement read_firmware(READ_FIRMWARE_VERSION);
        const std::vector<DataElement> response = SendFrameAndAwaitResponse(read_firmware, 0x01, 0.1);
        if (response.size() != 1)
        {
        ...
        const uint16_t build_number = DeserializeFixedSizePOD<uint16_t>(response_msg.Payload(), 2).first;
        return std::make_pair(std::make_pair(major_version, minor_version), build_number);
    }

    std::vector<AtiNetCanOemInterface::DataElement> AtiNetCanOemInterface::SendFrameAndAwaitResponse(const DataElement& command, const uint8_t num_response_frames, const double timeout)
    {
            ...
            while (response_frames.size() < num_response_frames)
            {
                ...
                if (read_size == CAN_MTU)
                {
                    if (frame.can_dlc <= CAN_MAX_DLEN)
                    {
                        ...
                        if (frame.can_dlc > 0)
                        {
                            payload.insert(payload.end(), frame.data, frame.data + frame.can_dlc);
                        }
