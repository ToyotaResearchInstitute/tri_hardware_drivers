...
template<typename T, typename Allocator=std::allocator<T>>
inline uint64_t SerializeMemcpyableVector(
    const std::vector<T, Allocator>& vec_to_serialize,
    std::vector<uint8_t>& buffer);

template<typename T, typename Allocator=std::allocator<T>>
inline std::pair<std::vector<T, Allocator>, uint64_t>
DeserializeMemcpyableVector(
    const std::vector<uint8_t>& buffer,
    const uint64_t current);
...
template<typename T, typename Allocator=std::allocator<T>>
inline uint64_t SerializeNetworkMemcpyableVector(
    const std::vector<T, Allocator>& vec_to_serialize,
    std::vector<uint8_t>& buffer);

template<typename T, typename Allocator=std::allocator<T>>
inline std::pair<std::vector<T, Allocator>, uint64_t>
DeserializeNetworkMemcpyableVector(const std::vector<uint8_t>& buffer,
                                   const uint64_t current);
    ...
    const std::vector<uint8_t>& buffer, const uint64_t current)
{
  ...
  if ((current + sizeof(temp_item)) > buffer.size())
  {
    ...
    const uint64_t current)
{
  ...
  std::vector<T, Allocator> deserialized(size);
