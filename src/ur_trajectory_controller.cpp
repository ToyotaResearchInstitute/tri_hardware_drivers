...
namespace lightweight_ur_interface
{
    ...
    public:
        ...
        URTrajectoryController(ros::NodeHandle& nh,
                             ...
                             const bool limit_acceleration) : nh_(nh)
        {
            ...
            trajectory_command_sub_ = nh_.subscribe(trajectory_command_topic, 1, &URTrajectoryController::TrajectoryCommandCallback, this);
            abort_server_ = nh_.advertiseService(abort_service, &URTrajectoryController::AbortCB, this);
        }
        ...
        void Loop(const double control_rate)
        {
            ...
            while (ros::ok())
            {
                ...
                if (iteration_count == supersample_rate)
                {
                    ...
                    {
                        const double elapsed_time = (ros::Time::now() - active_trajectory_start_time_).toSec();
                        if (elapsed_time <= active_trajectory_->getDuration())
                        {
                            const std::vector<double> current_target_position = EigenHelpers::EigenVectorXdToStdVectorDouble(active_trajectory_->getPosition(elapsed_time));
                            const std::vector<double> current_target_velocity = EigenHelpers::EigenVectorXdToStdVectorDouble(active_trajectory_->getVelocity(elapsed_time));
                            const std::vector<double> raw_config_correction = ComputeRawNextStepTrajectory(current_config_, current_velocities_, current_target_position, current_target_velocity, control_interval);
                            const std::vector<double> velocity_limited_config_correction = LimitCorrectionVelocities(raw_config_correction);
                            if (limit_acceleration_)
                            {
                                const std::vector<double> acceleration_limited_config_correction = LimitCorrectionAccelerations(velocity_limited_config_correction, previous_velocity_command, control_interval);
                                CommandVelocities(acceleration_limited_config_correction);
                                previous_velocity_command = acceleration_limited_config_correction;
                                PublishState(current_target_position, acceleration_limited_config_correction, current_config_, current_velocities_);
                            }
                            else
                            {
                                CommandVelocities(velocity_limited_config_correction);
                                previous_velocity_command = velocity_limited_config_correction;
                                PublishState(current_target_position, velocity_limited_config_correction, current_config_, current_velocities_);
                            }
                        }
                        else
                        {
                            ROS_INFO_ONCE_NAMED(ros::this_node::getName(), "Reached the end of the current trajectory @ %f seconds, switching to hold position", elapsed_time);
                            const std::vector<double> current_target_position = EigenHelpers::EigenVectorXdToStdVectorDouble(active_trajectory_->getPosition(active_trajectory_->getDuration()));
                            const std::vector<double> raw_config_correction = ComputeRawNextStepPosition(current_config_, current_target_position, control_interval);
                            const std::vector<double> velocity_limited_config_correction = LimitCorrectionVelocities(raw_config_correction);
                            if (limit_acceleration_)
                            {
                                const std::vector<double> acceleration_limited_config_correction = LimitCorrectionAccelerations(velocity_limited_config_correction, previous_velocity_command, control_interval);
        ...
        void PublishState(const std::vector<double>& target_config, const std::vector<double>& target_velocities, const std::vector<double>& current_config, const std::vector<double>& current_velocities)
        {
        ...
        std::vector<double> ComputeNewConfigErrorIntegral(const std::vector<double>& raw_config_error_integral) const
        {
            assert(raw_config_error_integral.size() == joint_names_.size());
            ...
            for (size_t idx = 0; idx < limited_config_error_integral.size(); idx++)
            {
                ...
                const double clamped_joint_config_error_integral = arc_helpers::ClampValue(joint_config_error_integral, -i_clamp, i_clamp);
                limited_config_error_integral[idx] = clamped_joint_config_error_integral;
            }
            return limited_config_error_integral;
        }

        std::vector<double> ComputeRawNextStepTrajectory(const std::vector<double>& current_position, const std::vector<double>& current_velocity, const std::vector<double>& target_position, const std::vector<double>& target_velocity, const double time_interval)
        {
            assert(joint_names_.size() == current_position.size());
            assert(joint_names_.size() == current_velocity.size());
            assert(joint_names_.size() == target_position.size());
            assert(joint_names_.size() == target_velocity.size());
            // Compute position error
            const std::vector<double> position_errors = EigenHelpers::Sub(target_position, current_position);
            // Compute the integral of position error & update the stored value
            const std::vector<double> config_error_integrals_update = EigenHelpers::Multiply(EigenHelpers::Add(EigenHelpers::Multiply(last_position_errors_, 0.5), EigenHelpers::Multiply(position_errors, 0.5)), time_interval);
            position_error_integrals_ = ComputeNewConfigErrorIntegral(EigenHelpers::Add(position_error_integrals_, config_error_integrals_update));
            // Compute the veloicty error (this is the d term of the trajectory controller)
            const std::vector<double> velocity_errors = EigenHelpers::Sub(target_velocity, current_velocity);
            // Update the stored position error
        ...
        std::vector<double> ComputeRawNextStepPosition(const std::vector<double>& current_config, const std::vector<double>& target_config, const double time_interval)
        {
            ...
            const std::vector<double> config_errors = EigenHelpers::Sub(target_config, current_config);
            ...
            const std::vector<double> config_error_integrals_update = EigenHelpers::Multiply(EigenHelpers::Add(EigenHelpers::Multiply(last_position_errors_, 0.5), EigenHelpers::Multiply(config_errors, 0.5)), time_interval);
            position_error_integrals_ = ComputeNewConfigErrorIntegral(EigenHelpers::Add(position_error_integrals_, config_error_integrals_update));
            // Compute the derivative of pose error
            const std::vector<double> config_error_derivatives = EigenHelpers::Divide(EigenHelpers::Sub(config_errors, last_position_errors_), time_interval);
        ...
        std::vector<double> LimitCorrectionVelocities(const std::vector<double>& raw_velocities) const
        {
            assert(raw_velocities.size() == joint_velocity_limits_.size());
            ...
            for (size_t idx = 0; idx < limited_velocities.size(); idx++)
            {
                ...
                const double limited_velocity = arc_helpers::ClampValue(raw_velocity, -velocity_limit, velocity_limit);
                limited_velocities[idx] = limited_velocity;
            }
            return limited_velocities;
        }

        std::vector<double> LimitCorrectionAccelerations(const std::vector<double>& desired_velocities, const std::vector<double>& current_velocities, const double timestep) const
        {
            assert(desired_velocities.size() == joint_acceleration_limits_.size());
            assert(current_velocities.size() == joint_acceleration_limits_.size());
            ...
            for (size_t idx = 0; idx < limited_velocities.size(); idx++)
            {
                ...
                const double desired_acceleration = (desired_velocity - current_velocity) / timestep;
                const double limited_acceleration = arc_helpers::ClampValue(desired_acceleration, -acceleration_limit, acceleration_limit);
                const double limited_velocity = current_velocity + (limited_acceleration * timestep);
                const double fully_limited_velocity = arc_helpers::ClampValue(limited_velocity, -std::abs(desired_velocity), std::abs(desired_velocity));
        ...
        void TrajectoryCommandCallback(trajectory_msgs::JointTrajectory trajectory)
        {
            ...
            if (SetsEqual(trajectory.joint_names, joint_names_))
            {
                ...
                {
                    const trajectory_msgs::JointTrajectoryPoint& current_point = trajectory.points[tdx];
                    if (current_point.positions.size() != joint_names_.size())
                    {
                        ROS_WARN_NAMED(ros::this_node::getName(), "Invalid JointTrajectoryPoint: %zu positions, %zu joints", current_point.positions.size(), joint_names_.size());
                    ...
                    for (size_t idx = 0; idx < joint_names_.size(); idx++)
                    {
                        ...
                        const std::pair<double, double>& joint_position_limit = joint_position_limits_[idx];
                        ...
                        if (found_itr != command_map.end())
                        {
                            ...
                            const double limited_position = arc_helpers::ClampValueAndWarn(position, joint_position_limit.first, joint_position_limit.second);
                            ordered_trajectory_point((ssize_t)idx) = limited_position;
                        }
                        else
                        {
                            ROS_WARN_NAMED(ros::this_node::getName(), "Invalid JointTrajectoryPoint: joint %s missing", joint_name.c_str());
                            trajectory_valid = false;
                            break;
                        }
                    }
            ...
            {
                const std::string provided_joint_names = PrettyPrint::PrettyPrint(trajectory.joint_names);
                const std::string our_joint_names = PrettyPrint::PrettyPrint(joint_names_);
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid JointTrajectory, provided joint names [%s] do not match ours [%s]", provided_joint_names.c_str(), our_joint_names.c_str());
                active_trajectory_.reset();
            }
        }
        ...
        void StateFeedbackCallback(sensor_msgs::JointState config_feedback)
        {
            if ((config_feedback.name.size() == config_feedback.position.size()) && (config_feedback.name.size() == config_feedback.velocity.size()) && IsSubset(config_feedback.name, joint_names_))
            {
                ...
                for (size_t idx = 0; idx < joint_names_.size(); idx++)
                {
                    ...
                    {
                        ROS_WARN_NAMED(ros::this_node::getName(), "Invalid JointState feedback: joint %s missing", joint_name.c_str());
                        config_valid = false;
                    }
                }
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO_NAMED(ros::this_node::getName(), "Starting ur_trajectory_controller...");
    ros::NodeHandle nh;
    ros::NodeHandle nhp("~");
    const std::string DEFAULT_TRAJECTORY_COMMAND_TOPIC = "/ur10/joint_command_trajectory";
    ...
    const std::string trajectory_command_topic = nhp.param(std::string("trajectory_command_topic"), DEFAULT_TRAJECTORY_COMMAND_TOPIC);
    const std::string state_feedback_topic = nhp.param(std::string("state_feedback_topic"), DEFAULT_STATE_FEEDBACK_TOPIC);
    ...
    const double base_kp = std::abs(nhp.param(std::string("base_kp"), DEFAULT_BASE_KP));
    const double base_kd = std::abs(nhp.param(std::string("base_kd"), DEFAULT_BASE_KD));
    const bool limit_acceleration = nhp.param(std::string("limit_acceleration"), DEFAULT_LIMIT_ACCELERATION);
    // Joint limits
    ...
    // Joint PID params
    const std::map<std::string, lightweight_ur_interface::PIDParams> joint_controller_params = lightweight_ur_interface::GetDefaultPositionControllerParams(base_kp, 0.0, base_kd, 0.0);
    lightweight_ur_interface::URTrajectoryController controller(nh, trajectory_command_topic, state_feedback_topic, velocity_command_topic, status_topic, abort_service, joint_limits, joint_controller_params, limit_acceleration);
