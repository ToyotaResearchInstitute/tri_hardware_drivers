...
{
    WSGUDPInterface::WSGUDPInterface(const std::function<void(const std::string&)>& logging_fn, const std::string& gripper_ip_address, const uint16_t gripper_port, const uint16_t local_port) : WSGInterface(logging_fn)
    {
        Log("Attempting to create WSG gripper UDP interface with gripper IP " + gripper_ip_address + " gripper port " + std::to_string(gripper_port) + " local port " + std::to_string(local_port));
        // Make the sockets
        ...
        const int bind_result = bind(recv_socket_fd_, (struct sockaddr *)&local_sockaddr_, sizeof(local_sockaddr_));
        if (bind_result != 0)
        {
            perror(NULL);
        ...
        active_.store(true);
        recv_thread_ = std::thread(std::bind(&WSGUDPInterface::RecvFromGripper, this));
    }
    ...
    bool WSGUDPInterface::CommandGripper(const WSGRawCommandMessage& command)
    {
        ...
        const size_t send_result = sendto(send_socket_fd_, serialized_command_buffer.data(), serialized_command_buffer.size(), 0, (struct sockaddr*)&gripper_sockaddr_, sizeof(gripper_sockaddr_));
    ...
    void WSGUDPInterface::RecvFromGripper()
    {
        ...
        while (active_.load())
        {
            const ssize_t read_size = recvfrom(recv_socket_fd_, recv_buffer.data(), recv_buffer.size(), MSG_DONTWAIT, (struct sockaddr*)&source_sockaddr, &source_sockaddr_len);
            if (read_size < 0)
            {
            ...
            else
            {
                ...
                serialized_message.insert(serialized_message.end(), recv_buffer.begin(), recv_buffer.begin() + read_size);
                const WSGRawStatusMessage status_msg = WSGRawStatusMessage::Deserialize(serialized_message, 0).first;
                std::lock_guard<std::mutex> lock(status_mutex_);
