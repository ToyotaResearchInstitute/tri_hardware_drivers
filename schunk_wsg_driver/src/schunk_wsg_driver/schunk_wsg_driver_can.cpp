...
{
    WSGCANInterface::WSGCANInterface(const std::function<void(const std::string&)>& logging_fn, const std::string& socketcan_interface, const uint32_t gripper_send_can_id) : WSGInterface(logging_fn), gripper_send_can_id_(gripper_send_can_id), gripper_recv_can_id_(gripper_send_can_id + 1u)
    {
        Log("Attempting to create WSG gripper CAN interface with socketcan interface " + socketcan_interface + " gripper base_can_id " + std::to_string(gripper_send_can_id));
        ...
        active_.store(true);
        recv_thread_ = std::thread(std::bind(&WSGCANInterface::RecvFromGripper, this));
    }
    ...
    bool WSGCANInterface::CommandGripper(const WSGRawCommandMessage& command)
    {
        ...
        const size_t num_frames = (size_t)ceil((double)serialized_command_size / (double)CAN_MAX_DLEN);
        ...
        for (size_t frame_num = 0; frame_num < num_frames; frame_num++)
        {
            ...
            const size_t bytes_to_write = std::min((size_t)CAN_MAX_DLEN, serialized_command_size - starting_offset);
            frame.can_dlc = (uint8_t)bytes_to_write;
            memcpy(frame.data, serialized_command_buffer.data() + starting_offset, bytes_to_write);
    ...
    void WSGCANInterface::RecvFromGripper()
    {
        ...
        while (active_.load())
        {
            ...
            if (read_size == CAN_MTU)
            {
                ...
                {
                    recv_buffer.insert(recv_buffer.end(), frame.data, frame.data + frame.can_dlc);
                }
                else if (frame.can_dlc < CAN_MAX_DLEN)
                {
                    recv_buffer.insert(recv_buffer.end(), frame.data, frame.data + frame.can_dlc);
                    const std::pair<WSGRawStatusMessage, uint64_t> deserialized_status_msg = WSGRawStatusMessage::Deserialize(recv_buffer, 0);
                    if (deserialized_status_msg.second < recv_buffer.size())
                    {
                        recv_buffer.erase(recv_buffer.begin(), recv_buffer.begin() + deserialized_status_msg.second);
                    }
                    else
                    {
                        recv_buffer.clear();
