...
namespace schunk_wsg_driver
{
    ...
    uint64_t WSGRawCommandMessage::Serialize(const WSGRawCommandMessage& message, std::vector<uint8_t>& buffer)
    {
    ...
    uint64_t WSGRawCommandMessage::SerializeSelf(std::vector<uint8_t>& buffer) const
    {
        ...
        const uint16_t checksum = ComputeCRC(buffer, start_buffer_size, buffer.size());
        // Add the checksum
    ...
    std::pair<WSGRawStatusMessage, uint64_t> WSGRawStatusMessage::Deserialize(const std::vector<uint8_t>& buffer, const uint64_t current)
    {
    ...
    uint64_t WSGRawStatusMessage::DeserializeSelf(const std::vector<uint8_t>& buffer, const uint64_t current)
    {
        uint64_t current_position = current;
        // Make sure the buffer is big enough
        if ((buffer.size() - current) < 8)
        {
            throw std::runtime_error("Buffer is not large enough to load message header");
        }
        ...
        {
            throw std::runtime_error("Message header is invalid - h1 != h2 != h3 != 0xaa");
        }
        // Load the command
        command_ = buffer[current_position + 3];
        // Load the payload size
        size_t payload_size = (size_t)buffer[current_position + 4] + ((size_t)buffer[current_position + 5] << 8);
        if (payload_size < 2)
        {
            throw std::runtime_error("Invalid message payload size < 2");
        }
        // Load the status code
        status_ = (uint16_t)(buffer[current_position + 6] + (buffer[current_position + 7] << 8));
        // Update current position
        current_position += 8;
        // Check to make sure the buffer is large enough, now that we know the payload size
        if ((buffer.size() - 8) < payload_size)
        {
            throw std::runtime_error("Buffer is not large enough to load message payload");
        }
        // Copy over the payload buffer
        const size_t checksum_size = 2;
        param_buffer_.clear();
        param_buffer_.insert(param_buffer_.end(), buffer.begin() + current_position, buffer.begin() + current_position + payload_size - checksum_size);
        current_position += (payload_size - checksum_size);
        // Read the checksum
        const uint16_t read_checksum = (uint16_t)(buffer[current_position] + (buffer[current_position + 1] << 8));
        // Validate the checksum
        const uint16_t computed_checksum = ComputeCRC(buffer, current, current_position);
        if (read_checksum != computed_checksum)
        {
            throw std::runtime_error("Received message checksum does not match computed checksum");
        }
        current_position += 2;
    ...
    std::pair<WSGRawStatusMessage, bool> WSGInterface::SendCommandAndAwaitStatus(const WSGRawCommandMessage& command, const double timeout)
    {
        ...
        std::pair<WSGRawStatusMessage, bool> response = std::make_pair(WSGRawStatusMessage(), false);
        while (!response.second)
        {
            ...
            for (size_t idx = 0; idx < status_queue.size(); idx++)
            {
                ...
                else
                {
                    ...
                    {
                        const std::string msg = "Non-success response: " + std::to_string(candidate_response.Status());
                        Log(msg);
                    }
                }
            }
    ...
    bool WSGInterface::Home(const HomeDirection direction)
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(home_command, 4.0);
    ...
    bool WSGInterface::Tare()
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(tare_command, 4.0);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::Grasp(const double width_mm, const double speed_mm_per_s)
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(grasp_command, 6.0);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::SetForceLimit(const double force)
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(force_limit_command, 0.1);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::SetAcceleration(const double acceleration_mm_per_s)
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(acceleration_command, 0.1);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::ClearSoftLimits()
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(clear_limits_command, 4.0);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::EnableRecurringStatus(const GripperCommand command, const uint16_t update_period_ms, const double timeout)
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(recurring_status_command, timeout);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::DisableRecurringStatus(const GripperCommand command, const double timeout)
    {
        WSGRawCommandMessage recurring_status_command(command);
    ...
    WSGRawCommandMessage WSGInterface::MakePrePositionCommand(const PrePositionStopMode stop_mode, const PrePositionMoveMode move_mode, double width_mm, double speed_mm_per_s) const
    {
    ...
    bool WSGInterface::PrePosition(const PrePositionStopMode stop_mode, const PrePositionMoveMode move_mode, double width_mm, double speed_mm_per_s)
    {
        const WSGRawCommandMessage preposition_command = MakePrePositionCommand(stop_mode, move_mode, width_mm, speed_mm_per_s);
        const std::pair<WSGRawStatusMessage, bool> maybe_response = SendCommandAndAwaitStatus(preposition_command, 6.0);
        if (maybe_response.second)
        {
            if (maybe_response.first.Status() == E_SUCCESS)
            {
    ...
    bool WSGInterface::PrePositionNonBlocking(const PrePositionStopMode stop_mode, const PrePositionMoveMode move_mode, double width_mm, double speed_mm_per_s)
    {
        const WSGRawCommandMessage preposition_command = MakePrePositionCommand(stop_mode, move_mode, width_mm, speed_mm_per_s);
    ...
    PhysicalLimits WSGInterface::GetGripperPhysicalLimits()
    {
        ...
        const std::pair<WSGRawStatusMessage, bool> maybe_status = SendCommandAndAwaitStatus(physical_limits_command, 0.1);
        ...
        const double stroke_mm = DeserializeFixedSizePOD<float>(param_buffer, 0).first;
        const double min_speed_mm_per_s = DeserializeFixedSizePOD<float>(param_buffer, 4).first;
        const double max_speed_mm_per_s = DeserializeFixedSizePOD<float>(param_buffer, 8).first;
        const double min_acc_mm_per_ss = DeserializeFixedSizePOD<float>(param_buffer, 12).first;
        const double max_acc_mm_per_ss = DeserializeFixedSizePOD<float>(param_buffer, 16).first;
        const double min_force = DeserializeFixedSizePOD<float>(param_buffer, 20).first;
        const double nominal_force = DeserializeFixedSizePOD<float>(param_buffer, 24).first;
        const double overdrive_force = DeserializeFixedSizePOD<float>(param_buffer, 28).first;
        Log("...loaded physical limits from gripper");
        return PhysicalLimits(stroke_mm, min_speed_mm_per_s, max_speed_mm_per_s, min_acc_mm_per_ss,
                              max_acc_mm_per_ss, min_force, nominal_force, overdrive_force);
    }
    ...
    bool WSGInterface::InitializeGripper()
    {
        ...
        success = success && EnableRecurringStatus(kGetSystemState, update_period_ms, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to enable recurring kGetSystemState");
        }
        success = success && EnableRecurringStatus(kGetGraspState, update_period_ms, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to enable recurring kGetGraspState");
        }
        success = success && EnableRecurringStatus(kGetOpeningWidth, update_period_ms, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to enable recurring kGetOpeningWidth");
        }
        success = success && EnableRecurringStatus(kGetSpeed, update_period_ms, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to enable recurring kGetSpeed");
        }
        success = success && EnableRecurringStatus(kGetForce, update_period_ms, update_adjust_timeout);
        if (!success)
        {
        ...
        success = success && SetAcceleration(limits.MinMaxAccelMMPerSS().second);
        if (!success)
        {
    ...
    void WSGInterface::Shutdown()
    {
        ...
        bool success = true;
        const double update_adjust_timeout = 0.25;
        success = success && StopGripper();
        if (!success)
        {
            throw std::runtime_error("Failed to stop gripper");
        }
        success = success && DisableRecurringStatus(kGetSystemState, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to disable recurring kGetSystemState");
        }
        success = success && DisableRecurringStatus(kGetGraspState, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to disable recurring kGetGraspState");
        }
        success = success && DisableRecurringStatus(kGetOpeningWidth, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to disable recurring kGetOpeningWidth");
        }
        success = success && DisableRecurringStatus(kGetSpeed, update_adjust_timeout);
        if (!success)
        {
            throw std::runtime_error("Failed to disable recurring kGetSpeed");
        }
        success = success && DisableRecurringStatus(kGetForce, update_adjust_timeout);
        if (!success)
        {
    ...
    bool WSGInterface::SetTargetPositionAndEffort(const double target_position, const double max_effort)
    {
        ...
        if (target_position > current_status.ActualPosition() && current_status.ActualPosition() > current_status.TargetPosition())
        {
            is_new_command = true;
        }
        else if (target_position < current_status.ActualPosition() && current_status.ActualPosition() < current_status.TargetPosition())
        {
            is_new_command = true;
        }
        if (is_new_command)
    ...
    void WSGInterface::RefreshGripperStatus()
    {
        ...
        for (size_t idx = 0; idx < status_queue.size(); idx++)
        {
            ...
            {
                const double opening_width_mm = DeserializeFixedSizePOD<float>(param_buffer, 0).first;
            ...
            {
                const double force = DeserializeFixedSizePOD<float>(param_buffer, 0).first;
                motion_status_.UpdateActualEffort(force);
            }
            else if (message.Command() == kGetSpeed)
            {
                const double speed_mm_s = DeserializeFixedSizePOD<float>(param_buffer, 0).first;
