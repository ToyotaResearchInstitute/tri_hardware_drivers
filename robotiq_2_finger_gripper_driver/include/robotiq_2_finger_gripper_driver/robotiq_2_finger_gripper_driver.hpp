...
#ifndef ROBOTIQ_2_FINGER_GRIPPER_DRIVER_HPP
#define ROBOTIQ_2_FINGER_GRIPPER_DRIVER_HPP
...
namespace robotiq_2_finger_gripper_driver
{
    ...
    public:
        ...
        Robotiq2FingerGripperStatus(const std::vector<uint8_t>& received_bytes)
        {
            ...
            gripper_activation_ = (GRIPPER_ACTIVATION)(raw_gripper_status_byte & GACT_MASK);
            action_status_ = (ACTION_STATUS)((raw_gripper_status_byte & GGTO_MASK) >> 3);
            gripper_status_ = (GRIPPER_STATUS)((raw_gripper_status_byte & GSTA_MASK) >> 4);
            object_status_ = (OBJECT_STATUS)((raw_gripper_status_byte & GOBJ_MASK) >> 6);
            if ((object_status_ == FINGERS_IN_MOTION) && (action_status_ == GRIPPER_STOPPED))
            {
        ...
        inline GRIPPER_ACTIVATION GripperActivation() const { return gripper_activation_; }

        inline ACTION_STATUS ActionStatus() const { return action_status_; }

        inline GRIPPER_STATUS GripperStatus() const { return gripper_status_; }

        inline OBJECT_STATUS ObjectStatus() const { return object_status_; }

        inline FAULT_STATUS FaultStatus() const { return fault_status_; }

        inline double TargetPosition() const { return target_position_; }

        inline double ActualPosition() const { return actual_position_; }

        inline double ActualCurrent() const { return actual_current_; }
        ...
        {
            if ((gripper_activation_ == GRIPPER_ACTIVATED) && (gripper_status_ == ACTIVATION_COMPLETED))
            {
                return true;
            }
            else
...
#endif // ROBOTIQ_2_FINGER_GRIPPER_DRIVER_HPP
