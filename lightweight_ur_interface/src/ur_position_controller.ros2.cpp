...
#include <lightweight_ur_interface/msg/position_command.hpp>
...
class URPositionControllerNode : public rclcpp::Node
{
private:
  ...
  bool target_config_valid_ = false;
  ...
  std::shared_ptr<
    rclcpp::Subscription<
      lightweight_ur_interface::msg::PositionCommand>> position_command_sub_;
  std::shared_ptr<rclcpp::Service<std_srvs::srv::Empty>> abort_service_;
  std::shared_ptr<rclcpp::TimerBase> control_timer_;

public:

  explicit URPositionControllerNode(const rclcpp::NodeOptions& options)
      : rclcpp::Node("ur_position_controller", options)
  {
    RCLCPP_INFO(this->get_logger(), "Starting ur_position_controller...");
    ...
    constexpr bool DEFAULT_ENABLE_VELOCITY_AUTOSCALING = true;
    ...
    autoscale_velocities_
        = this->declare_parameter("enable_velocity_autoscaling",
                                  DEFAULT_ENABLE_VELOCITY_AUTOSCALING);

    // Joint limits
    const double velocity_limit_scaling
        = this->declare_parameter("velocity_limit_scaling",
                                  DEFAULT_VELOCITY_LIMIT_SCALING);
    const double acceleration_limit_scaling
        = this->declare_parameter("acceleration_limit_scaling",
    ...
    feedback_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
        state_feedback_topic, 1,
        std::bind(&URPositionControllerNode::StateFeedbackCallback, this, _1));

    const std::string position_command_topic
        = this->declare_parameter("position_command_topic",
                                  DEFAULT_POSITION_COMMAND_TOPIC);
    auto position_command_callback =
        std::bind(&URPositionControllerNode::PositionCommandCallback, this, _1);
    position_command_sub_ = this->create_subscription<
      lightweight_ur_interface::msg::PositionCommand>(
          position_command_topic, 1, position_command_callback);
    ...
    abort_service_ = this->create_service<std_srvs::srv::Empty>(
        abort_service,
        std::bind(&URPositionControllerNode::AbortCB, this, _1, _2));
    ...
    control_timer_ = rclcpp::create_timer(
        ...
        std::bind(&URPositionControllerNode::SpinOnce, this));
    ...
    if (iteration_count_ == supersample_rate)
    {
      if (target_config_valid_ && current_state_valid_)
      {
        ...
        if (limit_acceleration_)
        {
          ...
          previous_velocity_command_ = acceleration_limited_config_correction;
        ...
        else
        {
          ...
          previous_velocity_command_ = velocity_limited_config_correction;
          PublishState(target_config_,
                       velocity_limited_config_correction,
                       current_config_,
                       current_velocities_);
...
private:
               ...
               std::shared_ptr<std_srvs::srv::Empty::Response> res)
  {
    UNUSED(req);
    UNUSED(res);
    RCLCPP_INFO(this->get_logger(), "Aborting position target");
  ...
  void PositionCommandCallback(
      const lightweight_ur_interface::msg::PositionCommand& config_target)
  {
    if (config_target.name.size() == config_target.position.size())
    {
      ...
      {
        RCLCPP_INFO(
            this->get_logger(), "Starting execution to a new target configuration");
  ...
  void StateFeedbackCallback(const sensor_msgs::msg::JointState& config_feedback)
  {
...
RCLCPP_COMPONENTS_REGISTER_NODE(lightweight_ur_interface::URPositionControllerNode)
