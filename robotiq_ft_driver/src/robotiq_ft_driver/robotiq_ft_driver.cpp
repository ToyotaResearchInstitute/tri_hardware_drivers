...
{
    RobotiqFTModbusRtuInterface::RobotiqFTModbusRtuInterface(const std::function<void(const std::string&)>& logging_fn, const std::string& modbus_rtu_interface, const uint16_t sensor_slave_id) : logging_fn_(logging_fn)
    {
        ...
        const int mss_ret = modbus_set_slave(modbus_interface_ptr_, (int)sensor_slave_id);
        if (mss_ret != 0)
        {
            const std::string error_msg(modbus_strerror(errno));
            throw std::runtime_error("Invalid Modbus slave address " + std::to_string(sensor_slave_id) + " with error: " + error_msg);
        }
    ...
    Eigen::Matrix<double, 6, 1> RobotiqFTModbusRtuInterface::GetCurrentForceTorque()
    {
        ...
        const int ret = modbus_read_registers(modbus_interface_ptr_, FX_REGISTER, 6, raw_ft_buffer.data());
        if (ret != 6)
        {
            const std::string error_msg(modbus_strerror(errno));
            throw std::runtime_error("Failed to read F/T registers with error: " + error_msg);
        }
        int16_t raw_fx = 0;
        int16_t raw_fy = 0;
        int16_t raw_fz = 0;
    ...
    Eigen::Vector3d RobotiqFTModbusRtuInterface::GetCurrentAcceleration()
    {
        ...
        const int ret = modbus_read_registers(modbus_interface_ptr_, AX_REGISTER, 3, raw_acc_buffer.data());
        if (ret != 3)
        {
            const std::string error_msg(modbus_strerror(errno));
            throw std::runtime_error("Failed to read Accelerometer registers with error: " + error_msg);
        }
        int16_t raw_ax = 0;
        int16_t raw_ay = 0;
        int16_t raw_az = 0;
    ...
    std::string RobotiqFTModbusRtuInterface::ReadSerialNumber()
    {
        ...
        const int ret = modbus_read_registers(modbus_interface_ptr_, SN1_REGISTER, 4, raw_sn_buffer.data());
        if (ret != 4)
        {
            const std::string error_msg(modbus_strerror(errno));
            throw std::runtime_error("Failed to read Serial Number registers with error: " + error_msg);
        }
        ...
        if (raw_sn > 0)
        {
            ...
            const int written = snprintf(sn_str_buffer, max_str_len - 1, "%c%c%c-%.4lu", (char)(raw_sn_buffer[0] >> 8), (char)(raw_sn_buffer[0] & 0xff), (char)(raw_sn_buffer[1] >> 8), raw_sn);
            assert(written > 0);
    ...
    std::string RobotiqFTModbusRtuInterface::ReadFirmwareVersion()
    {
        ...
        const int ret = modbus_read_registers(modbus_interface_ptr_, FW1_REGISTER, 3, raw_fw_buffer.data());
        if (ret != 3)
        {
            const std::string error_msg(modbus_strerror(errno));
            throw std::runtime_error("Failed to read Firmware registers with error: " + error_msg);
        }
        const int max_str_len = 128;
        char fw_str_buffer[max_str_len];
        const int written = snprintf(fw_str_buffer, max_str_len - 1, "%c%c%c-%hhu.%hhu.%hhu",
                                     (char)(raw_fw_buffer[0] >> 8), (char)(raw_fw_buffer[0] & 0xff),
                                     (char)(raw_fw_buffer[1] >> 8), (char)(raw_fw_buffer[1] & 0xff),
                                     (char)(raw_fw_buffer[2] >> 8), (char)(raw_fw_buffer[2] & 0xff));
        assert(written > 0);
        assert(written < max_str_len);
    ...
    {
        uint16_t yom;
        const int ret = modbus_read_registers(modbus_interface_ptr_, YOM_REGISTER, 1, &yom);
        if (ret != 1)
        {
            const std::string error_msg(modbus_strerror(errno));
            throw std::runtime_error("Failed to read YOM register with error: " + error_msg);
        }
        return yom;
    }
