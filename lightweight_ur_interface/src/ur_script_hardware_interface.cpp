...
#include <lightweight_ur_interface/ConfigureForceMode.h>
...
namespace lightweight_ur_interface
{
    ...
    protected:
        ...
        protected:
            ...
            static uint64_t SerializeKnownSizeVector(const std::vector<T, Allocator>& vec_to_serialize, std::vector<uint8_t>& buffer, const std::function<uint64_t(const T&, std::vector<uint8_t>&)>& item_serializer)
            {
            ...
            static uint64_t SerializeKnownSizeInt32Vector(const std::vector<int32_t>& vec_to_serialize, std::vector<uint8_t>& buffer)
            {
                return SerializeKnownSizeVector<int32_t>(vec_to_serialize, buffer, arc_helpers::SerializeNetworkFixedSizePOD<int32_t>);
            }
        ...
        public:
            ...
            static int64_t Serialize(const ControlScriptCommand& command, const double float_conversion_ratio, std::vector<uint8_t>& buffer)
            {
                return command.SerializeSelf(float_conversion_ratio, buffer);
            }

            static std::vector<int32_t> ConvertToInt32Vector(const std::vector<double>& vector, const double float_conversion_ratio)
            {
            ...
            int64_t SerializeSelf(const double float_conversion_ratio, std::vector<uint8_t>& buffer) const
            {
                const uint64_t start_buffer_size = buffer.size();
                // Serialize the contained items
                SerializeKnownSizeInt32Vector(ConvertToInt32Vector(speed_, float_conversion_ratio), buffer);
                SerializeKnownSizeInt32Vector(ConvertToInt32Vector(wrench_, float_conversion_ratio), buffer);
                SerializeKnownSizeInt32Vector(ConvertToInt32Vector(force_mode_limits_, float_conversion_ratio), buffer);
                SerializeKnownSizeInt32Vector(force_mode_selection_vector_, buffer);
                arc_helpers::SerializeNetworkFixedSizePOD<int32_t>((int32_t)control_mode_, buffer);
                arc_helpers::SerializeNetworkFixedSizePOD<int32_t>((int32_t)force_mode_, buffer);
                if (running_)
                {
                    arc_helpers::SerializeNetworkFixedSizePOD<int32_t>(1, buffer);
                }
                else
                {
                    arc_helpers::SerializeNetworkFixedSizePOD<int32_t>(0, buffer);
                }
                // Figure out how many bytes were written
                const uint64_t end_buffer_size = buffer.size();
                const uint64_t bytes_written = end_buffer_size - start_buffer_size;
            ...
            static ControlScriptCommand MakeSpeedJCommand(const std::vector<double>& joint_velocities)
            {
            ...
            static ControlScriptCommand MakeSpeedLCommand(const std::vector<double>& ee_velocities)
            {
                ControlScriptCommand command;
            ...
            static ControlScriptCommand MakeForceModeCommand(const std::vector<double>& wrench, const std::vector<double>& force_mode_limits, const std::vector<int32_t>& force_mode_selection_vector)
            {
                ControlScriptCommand command;
                if (wrench.size() != 6)
                {
            ...
            static ControlScriptCommand MakeWrenchModeCommand(const std::vector<double>& ee_velocities, const std::vector<double>& wrench)
            {
                ControlScriptCommand command;
                if (ee_velocities.size() != 6)
                {
    ...
    public:
        ...
        URScriptHardwareInterface(ros::NodeHandle& nh,
                                  ...
                                  const int32_t control_port) : nh_(nh)
        {
            ...
            velocity_command_sub_ = nh_.subscribe(velocity_command_topic, 1, &URScriptHardwareInterface::VelocityCommandCallback, this);
            twist_command_sub_ = nh_.subscribe(twist_command_topic, 1, &URScriptHardwareInterface::TwistCommandCallback, this);
            wrench_command_sub_ = nh_.subscribe(wrench_command_topic, 1, &URScriptHardwareInterface::WrenchCommandCallback, this);
            switch_force_mode_server_ = nh_.advertiseService(force_mode_service, &URScriptHardwareInterface::SwitchForceModeCB, this);
            switch_teach_mode_server_ = nh_.advertiseService(teach_mode_service, &URScriptHardwareInterface::SwitchTeachModeCB, this);
        ...
        void Run()
        {
            ...
            ROS_INFO_NAMED(ros::this_node::getName(), "Started robot realtime interface");
            // Start control program interface
            const std::pair<int32_t, int32_t> control_program_socket_fds = StartControlProgram(our_ip_address_,
                                                                                               control_port_,
            ...
            ROS_INFO_NAMED(ros::this_node::getName(), "Started robot control interface with fds %d, %d", control_program_incoming_sock_fd, control_program_sock_fd);
            // Start ROS spinloop
            ...
            while (nh_.ok())
            {
                ...
                {
                    const ControlScriptCommand& current_command = control_script_command_queue_[idx];
                    std::vector<uint8_t> buffer;
                    current_command.SerializeSelf(FLOAT_CONVERSION_RATIO, buffer);
                    const ssize_t bytes_written = write(control_program_sock_fd, buffer.data(), buffer.size());
                    if (bytes_written != (ssize_t)buffer.size())
                    {
                        perror(NULL);
                        ROS_ERROR_NAMED(ros::this_node::getName(), "Failed to send command with %zu bytes, sent %zd instead", buffer.size(), bytes_written);
                        ROS_INFO_NAMED(ros::this_node::getName(), "Trying to restart/reconnect to the robot control script...");
                        ...
                        ROS_INFO_NAMED(ros::this_node::getName(), "Restarted/reconnected robot control interface with fds %d, %d", control_program_incoming_sock_fd, control_program_sock_fd);
                    }
                }
                control_script_command_queue_.clear();
                looprate.sleep();
            }
            const ControlScriptCommand exit_command = ControlScriptCommand::MakeExitProgramCommand();
            std::vector<uint8_t> buffer;
        ...
        std::pair<int32_t, int32_t> StartControlProgram(const std::string& our_ip_address,
                                                        ...
                                                        const double speed_command_wait)
        {
            ...
            {
                throw std::runtime_error("ERROR opening socket for control program communication");
            }
            else
            {
                ROS_INFO_NAMED(ros::this_node::getName(), "Opened socket for control program communication");
            }
            ...
            int flag = 1;
            setsockopt(incoming_sock_fd, IPPROTO_TCP, TCP_NODELAY, (char *) &flag, sizeof(int));
            setsockopt(incoming_sock_fd, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(int));
            if (bind(incoming_sock_fd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
            {
                throw std::runtime_error("ERROR on binding socket for control program communication");
            }
            else
            {
                ROS_INFO_NAMED(ros::this_node::getName(), "Bound socket for control program communication");
            }
            listen(incoming_sock_fd, 5);
            struct sockaddr_in cli_addr;
            socklen_t clilen;
            clilen = sizeof(cli_addr);
            int new_sock_fd = accept(incoming_sock_fd, (struct sockaddr *) &cli_addr, &clilen);
            if (new_sock_fd < 0)
            {
                throw std::runtime_error("ERROR on accepting control program communication");
            }
            else
            {
                ROS_INFO_NAMED(ros::this_node::getName(), "Accepted connection for control program communication");
            }
        ...
        bool SwitchForceModeCB(lightweight_ur_interface::ConfigureForceMode::Request& req,
                               lightweight_ur_interface::ConfigureForceMode::Response& res)
        {
            if (req.selection_vector.size() == 6)
            {
                const std::vector<double> raw_wrench = {req.wrench.force.x, req.wrench.force.y, req.wrench.force.z,
                                                        req.wrench.torque.x, req.wrench.torque.y, req.wrench.torque.z};
                const std::vector<double> raw_limits = {req.limits.linear.x, req.limits.linear.y, req.limits.linear.z,
                                                        req.limits.angular.x, req.limits.angular.y, req.limits.angular.z};
                if (in_force_mode_.load() == false)
                {
                    control_script_command_queue_.push_back(ControlScriptCommand::MakeForceModeCommand(raw_wrench, raw_limits, req.selection_vector));
                ...
                {
                    control_script_command_queue_.push_back(ControlScriptCommand::MakeExitForceModeCommand());
                    control_script_command_queue_.push_back(ControlScriptCommand::MakeForceModeCommand(raw_wrench, raw_limits, req.selection_vector));
                    res.success = true;
                    res.message = "Already in force mode, rebuilt force mode with new settings";
                }
            }
            else
            {
        ...
        bool SwitchTeachModeCB(std_srvs::SetBool::Request& req, std_srvs::SetBool::Response& res)
        {
            if (req.data)
            {
                ...
                {
                    control_script_command_queue_.push_back(ControlScriptCommand::MakeTeachModeCommand());
            ...
            else
            {
                ...
                {
                    control_script_command_queue_.push_back(ControlScriptCommand::MakeExitTeachModeCommand());
                    in_teach_mode_.store(false);
                    res.success = true;
        ...
        void VelocityCommandCallback(lightweight_ur_interface::VelocityCommand config_target)
        {
            if (config_target.name.size() == config_target.velocity.size())
            {
                ...
                if (command_valid)
                {
                    if (in_teach_mode_.load() == false)
                    {
                        control_script_command_queue_.push_back(ControlScriptCommand::MakeSpeedJCommand(target_velocity));
                    }
                    else
                    {
                ...
                if (in_teach_mode_.load() == false)
                {
                    if (twist_command.header.frame_id == ee_frame_)
                    {
                        ...
                        if (valid_latest_state)
                        {
                            ...
                            control_script_command_queue_.push_back(ControlScriptCommand::MakeSpeedLCommand(base_frame_twist));
                        }
                        else
                        {
                    ...
                    {
                        control_script_command_queue_.push_back(ControlScriptCommand::MakeSpeedLCommand(raw_twist));
                    }
                    else
                    {
        ...
        void WrenchCommandCallback(lightweight_ur_interface::WrenchCommand wrench_command)
        {
            bool valid_wrench = true;
            if (std::isinf(wrench_command.ee_velocities.linear.x) || std::isnan(wrench_command.ee_velocities.linear.x))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, linear.x is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.ee_velocities.linear.y) || std::isnan(wrench_command.ee_velocities.linear.y))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, linear.y is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.ee_velocities.linear.z) || std::isnan(wrench_command.ee_velocities.linear.z))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, linear.z is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.ee_velocities.angular.x) || std::isnan(wrench_command.ee_velocities.angular.x))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, angular.x is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.ee_velocities.angular.y) || std::isnan(wrench_command.ee_velocities.angular.y))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, angular.y is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.ee_velocities.angular.z) || std::isnan(wrench_command.ee_velocities.angular.z))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, angular.z is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.applied_wrench.force.x) || std::isnan(wrench_command.applied_wrench.force.x))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, force.x is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.applied_wrench.force.y) || std::isnan(wrench_command.applied_wrench.force.y))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, force.y is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.applied_wrench.force.z) || std::isnan(wrench_command.applied_wrench.force.z))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, force.z is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.applied_wrench.torque.x) || std::isnan(wrench_command.applied_wrench.torque.x))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, torque.x is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.applied_wrench.torque.y) || std::isnan(wrench_command.applied_wrench.torque.y))
            {
                ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench command, torque.y is NAN or INF");
                valid_wrench = false;
            }
            if (std::isinf(wrench_command.applied_wrench.torque.z) || std::isnan(wrench_command.applied_wrench.torque.z))
            {
            ...
            if (valid_wrench)
            {
                const std::vector<double> raw_twist = {wrench_command.ee_velocities.linear.x,
                                                       wrench_command.ee_velocities.linear.y,
                                                       wrench_command.ee_velocities.linear.z,
                                                       wrench_command.ee_velocities.angular.x,
                                                       wrench_command.ee_velocities.angular.y,
                                                       wrench_command.ee_velocities.angular.z};
                const std::vector<double> raw_wrench = {wrench_command.applied_wrench.force.x,
                                                        wrench_command.applied_wrench.force.y,
                                                        wrench_command.applied_wrench.force.z,
                                                        wrench_command.applied_wrench.torque.x,
                                                        wrench_command.applied_wrench.torque.y,
                                                        wrench_command.applied_wrench.torque.z};
                if (in_teach_mode_.load() == false)
                {
                    if (in_force_mode_.load() == true)
                    {
                        if (wrench_command.header.frame_id == ee_frame_)
                        {
                            ...
                            if (valid_latest_state)
                            {
                                ...
                                const std::vector<double> base_frame_twist = {base_frame_linear_velocity.x(),
                                                                              ...
                                                                              base_frame_angular_velocity.y(),
                                                                              base_frame_angular_velocity.z()};
                                control_script_command_queue_.push_back(ControlScriptCommand::MakeWrenchModeCommand(base_frame_twist, raw_wrench));
                            }
                            else
                            {
                                ROS_WARN_NAMED(ros::this_node::getName(), "Ignoring Wrench since latest state is not valid");
                            }
                        }
                        else
                        {
                            ROS_WARN_NAMED(ros::this_node::getName(), "Invalid Wrench frame: got [%s] needs [%s]",
                                           wrench_command.header.frame_id.c_str(), ee_frame_.c_str());
                        }
                    }
                    else
                    {
                        ROS_WARN_NAMED(ros::this_node::getName(), "Ignoring Wrench since robot is not in force mode");
                    }
                }
                else
                {
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO_NAMED(ros::this_node::getName(), "Starting ur_script_hardware_interface...");
    ros::NodeHandle nh;
    ros::NodeHandle nhp("~");
    ...
    const std::string ee_frame = nhp.param(std::string("ee_wrench_frame"), DEFAULT_EE_FRAME);
    ...
    const std::string our_ip_address = nhp.param(std::string("our_ip_address"), DEFAULT_OUR_IP_ADDRESS);
    const int32_t control_port = std::abs(nhp.param(std::string("control_port"), DEFAULT_CONTROL_PORT));
    ...
    lightweight_ur_interface::URScriptHardwareInterface interface(nh, velocity_command_topic, twist_command_topic, wrench_command_topic, joint_state_topic, ee_pose_topic, ee_world_twist_topic, ee_body_twist_topic, ee_wrench_topic, base_frame, ee_frame, force_mode_service, teach_mode_service, ordered_joint_names, joint_limits, robot_hostname, our_ip_address, control_port);
